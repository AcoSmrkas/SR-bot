...
function getNetworkPrefix(networkType: NetworkType): ergo.NetworkPrefix {
    return networkType === 'testnet' ? ergo.NetworkPrefix.Testnet : ergo.NetworkPrefix.Mainnet;
}

export function getAddressFromMnemonic(mnemonic: string, mnemonicpass: string, networkType: NetworkType): string {
    ...
    return address.to_base58(getNetworkPrefix(networkType));
}
